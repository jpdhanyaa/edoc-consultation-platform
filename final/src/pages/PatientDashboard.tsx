...
export default function PatientDashboard({ user }: { user: AppUser }) {
  ...
  const [activeView, setActiveView] = useState<'overview' | 'appointments' | 'doctors' | 'records' | 'history' | 'prescriptions' | 'vitals' | 'ai-mentor' | 'agenda' | 'library'>('overview');
  ...
  const triggerEmergency = async () => {
    if (!window.confirm("ARE YOU SURE? This will send an IMMEDIATE EMERGENCY ALERT to your doctor and play an alarm sound.")) return;
    
    // Find last approved doctor
    const lastApt = appointments.find(a => a.status === 'approved' || a.status === 'completed');
    if (!lastApt) {
      alert("No linked medical professional found to alert. Please contact emergency services directly.");
      return;
    }
    ...
    try {
      ...
      alert("Emergency signal sent. Please remain calm. Help is being notified.");
    } catch (err) {
    ...
    <div className="max-w-6xl mx-auto space-y-8">
           ...
           >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                 {[
                   ...
                   { label: 'AI Mentor', value: 'Neural Ready', icon: Sparkles, color: 'text-indigo-600', bg: 'bg-indigo-50' },
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                 ...
                 <div className="space-y-8">
                    <div className="bg-indigo-600 p-8 rounded-[3rem] text-white shadow-xl shadow-indigo-100 overflow-hidden relative group">
                       <div className="absolute top-0 right-0 -mr-10 -mt-10 p-16 bg-white/10 rounded-full blur-2xl group-hover:scale-110 transition-all" />
                       <Sparkles className="h-10 w-10 mb-4 opacity-50" />
                       <h4 className="text-xl font-black mb-2 leading-tight">Gemini AI Health Mentor</h4>
                       <p className="text-indigo-100 text-[11px] font-bold leading-relaxed mb-6 opacity-80">AI-powered wellness insights tailored to your biometrics and medical history.</p>
                       <button 
                         onClick={() => setActiveView('ai-mentor')}
                         className="w-full py-4 bg-white text-indigo-600 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg hover:shadow-indigo-400/20 active:scale-95 transition-all"
                       >
                          Analyze My Profile
                       </button>
                    </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 space-y-6">
                       <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => setActiveView('agenda')} className="text-[10px] font-black text-blue-600 hover:underline">Vew All</button>
                       </div>
                       <div className="space-y-4">
                          {[
                            { title: 'Morning Insulin', time: '8:00 AM' },
                            { title: 'Hydration Check', time: '11:30 AM' },
                            { title: 'Walking Goal', time: '5:00 PM' }
                          ].map((task, i) => (
                             <div key={i} className="flex gap-4 p-4 border border-slate-50 rounded-2xl bg-slate-50/30 group hover:border-blue-100 transition-all">
                                <div className="w-2 h-2 rounded-full bg-blue-500 mt-1.5" />
                                <div>
                                   <p className="text-xs font-black text-slate-800">{task.title}</p>
                                   <p className="text-[9px] font-bold text-slate-400 uppercase">{task.time}</p>
                                </div>
                             </div>
                          ))}
                       </div>
                               ...
                               <button 
                                 onClick={async () => {
                                    try {
                                       ...
                                       alert("Payment successful! Prescription is now verified.");
                                    } catch (err) {
      ...
      {showBooking && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4" onClick={() => setShowBooking(false)}>
           ...
           >
              <div className="p-8">
                 <h2 className="text-2xl font-bold text-slate-800 mb-2">Request Consultation</h2>
                 <p className="text-slate-500 text-sm mb-8">Please fill in the details for your medical request.</p>
                 
                 <form onSubmit={handleBooking} className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                       <div className="space-y-2">
                          <label className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">Preferred Date</label>
                          <input type="date" required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 font-medium" value={date} onChange={e => setDate(e.target.value)} />
                       </div>
                       <div className="space-y-2">
                          <label className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">Preferred Time</label>
                          <input type="time" required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 font-medium" value={time} onChange={e => setTime(e.target.value)} />
                       </div>
                    </div>
                    <div className="space-y-2">
                       <label className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">Reason for Visit</label>
                       <textarea 
                          placeholder="Briefly describe your symptoms or concern..."
                          required
                          className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 font-medium h-32 resize-none"
                          value={reason}
                          onChange={e => setReason(e.target.value)}
                       />
                    </div>
                    <div className="flex gap-4 pt-4">
                       <button type="button" onClick={() => setShowBooking(false)} className="flex-1 py-4 font-bold text-slate-500 hover:bg-slate-50 rounded-2xl transition-colors">Cancel</button>
                       <button type="submit" className="flex-1 py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-xl shadow-blue-200 hover:bg-blue-700 transition-all">Submit Request</button>
                    </div>
                 </form>
              </div>
           </motion.div>
