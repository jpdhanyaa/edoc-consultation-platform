...
import { Calendar, Clock, MessageSquare, Video, CheckCircle2, XCircle, User, Users, ClipboardList, Search, ShieldAlert, Activity, Bell, FileText, ChevronRight, Plus, Send, Pill, Zap, Sparkles, LayoutList, BookOpen, TrendingUp } from 'lucide-react';
...
export default function DoctorDashboard({ user }: { user: AppUser }) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'appointments' | 'patients' | 'history' | 'prescriptions' | 'ai-copilot' | 'agenda' | 'broadcasts'>('overview');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      if (!snapshot.empty) {
        const emergency = { id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as Emergency;
        setActiveEmergency(emergency);
        
        // Play alert sound logic (Web Audio API or simple HTMLAudioElement)
        try {
  ...
  const fetchPatients = async (ids: string[]) => {
    try {
      ...
      console.warn("Could not fetch patient profiles:", err);
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-100 pb-10">
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-2xl w-full md:w-auto overflow-x-auto no-scrollbar">
           ...
           <button
             ...
             className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
               activeTab === 'appointments' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'
             }`}
           >
           ...
           <button
             ...
             className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
               activeTab === 'patients' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'
             }`}
           >
           ...
           <button
             ...
             className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
               activeTab === 'prescriptions' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'
             }`}
           >
           ...
           <button
             ...
             className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
               activeTab === 'ai-copilot' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'
             }`}
           >
           ...
           <button
             ...
             className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
               activeTab === 'agenda' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'
             }`}
           >
           ...
           <button
             ...
             className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
               activeTab === 'broadcasts' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'
             }`}
           >
        ...
        {activeTab === 'overview' ? (
           ...
           >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                 {[
                   ...
                   { label: 'Clinical Tasks', value: 'Ready', icon: LayoutList, color: 'text-amber-600', bg: 'bg-amber-50' },
                   { label: 'AI Copilot', value: 'Neural On', icon: Sparkles, color: 'text-indigo-600', bg: 'bg-indigo-50' },
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                 ...
                 <div className="lg:col-span-2 space-y-8">
                    ...
                    {/* AI Insights Card */}
                    <div className="bg-slate-900 rounded-[3rem] p-10 text-white relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
                       <div className="absolute top-0 right-0 w-80 h-80 bg-indigo-600/20 rounded-full blur-3xl -mr-20 -mt-20" />
                       <div className="w-24 h-24 bg-indigo-600 rounded-[2.5rem] flex items-center justify-center text-white shadow-2xl flex-shrink-0 animate-pulse">
                          <Sparkles className="h-10 w-10" />
                       </div>
                       <div className="flex-1 text-center md:text-left relative z-10">
                          <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-2 font-black">AI Clinical Summary</p>
                          <h4 className="text-xl font-bold mb-4">Patient Population Trends</h4>
                          <p className="text-slate-400 text-sm leading-relaxed mb-6">Based on recent entries, 12% of your chronic patients show improved metabolic stability since last cycle.</p>
                          <button 
                            onClick={() => setActiveTab('ai-copilot')}
                            className="px-8 py-3 bg-white text-slate-900 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-xl"
                          >
                             Full Neural Audit
                          </button>
                       </div>
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 space-y-6">
                       {[
                         { title: 'Review Lab Results', patient: 'Sarah J.', color: 'border-blue-500' },
                         { title: 'Update Care Plan', patient: 'Emma W.', color: 'border-amber-500' },
                         { title: 'Authorize PX', patient: 'Tom H.', color: 'border-emerald-500' }
                       ].map((task, i) => (
                          <div key={i} className={`p-4 border-l-4 ${task.color} bg-slate-50/50 rounded-r-2xl`}>
                             <p className="text-xs font-black text-slate-900">{task.title}</p>
                             <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">{task.patient}</p>
                          </div>
                       ))}
                       <button onClick={() => setActiveTab('agenda')} className="w-full text-center text-xs font-black text-blue-600 uppercase tracking-widest hover:underline pt-2">Manage All Tasks</button>
                    </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8">
                       ...
                       <div className="space-y-6">
                          {prescriptions.slice(0, 3).map(px => (
                             <div key={px.id} className="flex gap-4">
                                <div className="w-10 h-10 bg-indigo-50 rounded-xl flex-shrink-0 flex items-center justify-center text-indigo-600">
                                   <Pill className="h-5 w-5" />
                                </div>
                                <div className="flex-1 min-w-0">
                                   <p className="text-sm font-bold text-slate-800 truncate">{px.medication} for {px.patientName}</p>
                                   <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">
                                      {px.isPaid ? 'Verified Payment' : 'Awaiting Settlement'}
                                   </p>
                                </div>
                             </div>
                          ))}
                          {prescriptions.length === 0 && (
                             <p className="text-xs text-slate-400 text-center py-4 italic">No recent activity detected.</p>
                          )}
                       </div>
                    </div>
                 </div>
          ...
          >
             <div className="p-8 border-b border-slate-50 flex items-center justify-between">
                <div>
                   <h3 className="text-xl font-bold text-slate-800">Issued Prescriptions</h3>
                   <p className="text-sm text-slate-400">Manage all digital scripts sent to patients.</p>
                </div>
                <div className="flex bg-slate-50 p-1 rounded-xl">
                   <div className="px-4 py-2 text-xs font-bold text-blue-600 bg-white rounded-lg shadow-sm">Total: {prescriptions.length}</div>
                </div>
             </div>
                      ...
                      {prescriptions.map(px => (
                         <tr key={px.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="px-8 py-5 text-right">
                               <button 
                                 onClick={() => setViewingProfile(px.patientId)}
          ...
          <motion.div 
            ...
            exit={{ opacity: 0, x: -20 }}
          >
             <AIConsultant user={user} context={`Active patients: ${patients.length}. Recent prescriptions: ${prescriptions.length}. Appointments: ${appointments.length}. System clinical load: Moderate.`} />
          </motion.div>
        ) : activeTab === 'agenda' ? (
          <motion.div 
            key="doc-agenda"
          ...
          >
             <div className="flex flex-col items-center justify-center py-10">
                ...
                <h3 className="text-xl font-bold text-slate-800">Hospital Master Registry</h3>
                <p className="text-sm text-slate-400 mt-1">Select a patient from the 'Patients' tab to view individual records.</p>
                <button onClick={() => setActiveTab('patients')} className="mt-6 px-6 py-3 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all">Go to Patient List</button>
             </div>
          </motion.div>
        )}
      </AnimatePresence>
                         ...
                         <div className="flex justify-end">
                            <button 
                              onClick={async () => {
                                 ...
                                 if (!med) return alert("Medication name is required");
