...
import { Send, Video, VideoOff, Mic, MicOff, PhoneOff, MessageSquare, ChevronLeft, User, FileText, ClipboardCheck, Info, Clock, Activity, ShieldCheck } from 'lucide-react';
  ...
  const savePrescription = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'prescriptions'), {
        ...
        isPaid: false,
        amount: 25.00, // Standard fee
        createdAt: new Date().toISOString(),
        appointmentId: appointment.id
      });

      alert('Prescription saved, session completed, and billable scrip issued!');
    } catch (err) {
      ...
      <div className="flex-1 bg-slate-900 rounded-[2.5rem] flex flex-col relative overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 flex items-center justify-center relative">
           <div className="text-center space-y-6">
              <div className="w-24 h-24 bg-slate-800 rounded-full flex items-center justify-center mx-auto shadow-inner relative">
                 <User className="h-12 w-12 text-slate-600" />
                 {isVideoOn && <div className="absolute inset-0 bg-blue-500/10 rounded-full animate-ping" />}
              </div>
              <p className="text-slate-500 font-bold tracking-tight uppercase text-xs">Waiting for video stream...</p>
           </div>
           
           <div className="absolute bottom-6 right-6 w-36 h-48 bg-slate-800 rounded-3xl border-2 border-slate-700 shadow-2xl flex items-center justify-center overflow-hidden">
